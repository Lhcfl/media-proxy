...
const lookup: LookupFunction = (hostname, options, callback) => {
	resolveHost(hostname).then(
		(addresses) => {
			let list = addresses;
			if (options?.family === 4) list = addresses.filter((a) => a.family === 4);
			else if (options?.family === 6)
				list = addresses.filter((a) => a.family === 6);
...
function requestDirect(
	...
	return new Promise((resolve, reject) => {
		...
		const req = mod.request(
			...
			requestOptions(
				config,
				isHttps ? { agent: httpsAgent } : { agent: httpAgent },
			),
			(res) => {
				resolve({
...
function connectTunnel(
	...
	return new Promise((resolve, reject) => {
		...
		req.setTimeout(config.timeout ?? 30_000, () => {
			req.destroy(
				new StatusError("Proxy connect timed out", 504, "Gateway Timeout"),
			);
		});
		req.on("connect", (res, socket) => {
			if (res.statusCode !== 200) {
				...
				reject(
					new StatusError(
						`Proxy CONNECT failed: ${res.statusCode}`,
						502,
						"Proxy Error",
					),
				);
...
async function requestViaProxyHttps(
	target: URL,
	proxy: ProxyConfig,
	config: DownloadConfig,
): Promise<HttpResult> {
...
export async function downloadUrl(
	...
	const overallTimer = setTimeout(() => {
		activeRequest?.destroy(
			new StatusError("Download timed out", 504, "Gateway Timeout"),
		);
	}, operationTimeout);
	...
	try {
		for (let redirect = 0; ; redirect++) {
			if (redirect > 10)
				throw new StatusError("Too many redirects", 508, "Loop Detected");

			if (proxy)
				await assertPublicHost(
					current.hostname,
					settings.allowedPrivateNetworks,
				);
			...
			if (
				!proxy &&
				remoteIp &&
				isBlockedAddress(remoteIp, settings.allowedPrivateNetworks)
			) {
				result.stream.destroy();
				throw new StatusError(
					`Blocked address: ${remoteIp}`,
					403,
					"Blocked address",
				);
			}
